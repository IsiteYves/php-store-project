...
if(!$db_connection) {
    ...
    $query = "SELECT * FROM stk_inventory WHERE inventory_id=$inventory_id";
...
<head>
    ...
    <title>Update an Inventory</title>
    <style>
        form {
            max-height: 64vh;
            overflow: auto;
...
<body>
    <div class="container">
        ...
        <h4>Update Inventory with id <?= $inventory_id ?>:</h4>
        <form action="./finish_inventory_update.php?id=<?= $inventory_id ?>" method="POST" autocomplete="off">
            ...
            <div class="submission-btn-holder">
                <input type="submit" name="product-create-button" value="Update Inventory" />
            </div>
