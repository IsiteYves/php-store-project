...
<head>
    ...
    <title>All products with quantities taken in store</title>
</head>
...
<body>
    <div class="container">
        ...
        <h4>All products with quantities taken in store:</h4>
        <?php
...
if (!$db_connection) {
    ...
    while($row = mysqli_fetch_array($db_result)){
        ...
        while(list($pName) = mysqli_fetch_array($db_result2)){
            array_push($products_quantities, array($row['outgoingId'], $row['productId'], $pName, $row['quantity']));
        }
    }
